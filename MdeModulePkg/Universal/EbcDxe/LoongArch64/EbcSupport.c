/** @file
  ...
  the target LoongArch64 processor.
...
#define ENTRY_POINT_TRANSFER_REG "r20" // t8
  ...
  )
{
  ...
  *(UINTN *)(UINTN)(VmContext.Gpr[0]) = (UINTN)VM_STACK_KEY_VALUE;
  ...
  // LoongArch64 does not do this so pad the stack accordingly.
  //
  PushU64 (&VmContext, (UINT64)0);
  PushU64 (&VmContext, (UINT64)0x1234567887654321ULL);

  //
  // For LoongArch64, this is where we say our return address is
  //
...
/**
  ...
  @param  Arg7                  The 7th argument.
  @param  Arg8                  The 8th argument.
  @param  ReturnAddr            The entrypoint of EBC interpreter.
...
{
  register UINTN EntryPoint asm(ENTRY_POINT_TRANSFER_REG);
  __asm__ __volatile__("" : "=r"(EntryPoint));
  __builtin_frob_return_addr(ReturnAddr);
  return EbcInterpret(Arg1, Arg2, Arg3, Arg4, Arg5, Arg6, Arg7, Arg8,
                      EntryPoint, __builtin_stack_address() + 16);
}
  ...
  )
{
  register UINTN EntryPoint asm(ENTRY_POINT_TRANSFER_REG);
  __asm__ __volatile__("" : "=r"(EntryPoint));
  return ExecuteEbcImageEntryPoint(ImageHandle, SystemTable, EntryPoint);
}
